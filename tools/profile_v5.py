...
baseline_spec=importlib.util.spec_from_file_location('v4_baseline',ROOT/'submission/script.py')
...
def hashes():
    paths = list((ROOT / 'submission_v5').glob('*.py')) + list((ROOT / 'submission_v5/model').glob('*.json'))
...
def main():
    ...
    parser.add_argument('--output', default='analysis/v5_prompt_profile.json')
    args = parser.parse_args()
    before = hashes()
    runner = TokenCounter()
    assets = script.Assets(ROOT / 'open/data', ROOT / 'submission_v5/model')
    ...
    review_path = ROOT / 'submission_v5/model/reviewed_examples.json'
    ...
    for rec in recs:
        for name, items, budget in script.GROUPS:
            ...
            assert tokens <= budget + (1536 if name=='B' else 0)
            old_messages,old_spans,old_ids,old_tokens=baseline.fit_messages(rec,assets,runner,args.max_chars,args.examples,items=items,budget=budget)
            assert spans == old_spans and ids == old_ids
            assert messages[1:] == old_messages[1:]
            assert messages[0]['content'] == old_messages[0]['content'] + (script.FACT_INSTRUCTION if name=='B' else '')
            assert tokens + script.output_limit(items) <= 16384
            for s in spans:
                assert rec['docs'][s['doc']]['text'][s['start']:s['end']] == s['text']
    ...
    report = {'type': 'fixed-tokenizer input/retrieval diagnostic only; no model, no F1',
        ...
        'groups': script.GROUPS, 'records': len(recs), 'requests': len(calls), 'v4_source_examples_user_messages_identical': True,
