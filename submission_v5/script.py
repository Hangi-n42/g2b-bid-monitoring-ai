...
B_ITEMS = tuple(range(10, 19))
# v5-F: preserve v4 selected source/examples, add only a B-group fact contract.
FACT_STATES = {
    'legal_context': ('national', 'local', 'other', 'conflict', 'unknown'),
    'actual_target': ('goods', 'service', 'construction', 'mixed', 'unknown'),
    'amounts': ('estimated_price_explicit', 'budget_only', 'conflict', 'unknown'),
    'product_match': ('actual_target_listed', 'qualification_code_only', 'name_candidate_only', 'actual_target_not_listed', 'conflict', 'unknown'),
    'for_profit_scope': ('small_only', 'sme', 'unrestricted', 'other_restriction', 'conflict', 'unknown'),
    'exception_scope': ('nonprofit_only', 'whole_contract', 'other_limited', 'not_observed', 'conflict', 'unknown'),
    'sme_requirement': ('mandatory', 'documents_only', 'not_observed', 'conflict', 'unknown'),
    'direct_requirement': ('mandatory', 'documents_only', 'not_observed', 'conflict', 'unknown'),
}
FACT_INSTRUCTION = '''
[B그룹 사실 확인 후 판정]
최상위 JSON은 facts를 먼저 출력하고 이어서 v10~v18을 출력한다. facts의 각 값은 state와 source만 가진다.
legal_context: national=국가계약, local=지방계약, other=그 외.
actual_target: 실제 구매대상 goods/service/construction/mixed. 참가자격 코드로 과업을 바꾸지 않는다.
amounts: estimated_price_explicit=추정가격 명시, budget_only=예산만 확인. 예산·추정가격·실적금액과 부가세를 구별해 원문 금액으로 항목별 기준을 적용한다.
product_match: actual_target_listed=현재 과업이 고시 대상이며 특이사항도 충족, qualification_code_only=자격요구 코드만 일치, name_candidate_only=품명 후보만 일치, actual_target_not_listed=비해당 확인. 단순 미검색은 unknown.
for_profit_scope: 영리기업 범위 small_only=소기업·소상공인만, sme=중소기업까지, unrestricted=규모 제한 없음, other_restriction=다른 제한.
exception_scope: nonprofit_only=비영리 주체만 예외, whole_contract=계약 전체에 적용되는 예외, other_limited=기타 한정 예외. 비영리 예외를 영리기업에 확장하지 않는다.
sme_requirement/direct_requirement: 중소기업 등 기업규모/직접생산 자격 각각 mandatory=필수 참가요건 명시, documents_only=서류·확인절차 언급만, not_observed=선택 문맥에서 못 찾음.
모든 필드에서 불명확하면 unknown, 상충을 해소하지 못하면 허용된 conflict 또는 unknown. not_observed는 전체 문서의 부재 확정이 아니다. 관측 상태와 항목별 적용대상·예외를 별도로 검토한다.
source는 해당 사실의 현재 원문 S번호 문자열 또는 meta:키, 없으면 unknown. 다른 사례의 근거를 쓰지 않는다. 사실의 source는 최종 evidence와 별개다.
facts는 잠정 관찰이며 원문과 충돌하면 원문을 다시 확인한다. 사실 상태만으로 위반을 기계적으로 확정하지 말고 기존 항목별 기준을 적용한다. 최종 정상·부재형 evidence=0.
'''


def uses_facts(items):
    return item_ids(items) == B_ITEMS


def output_limit(items):
    return 768 if uses_facts(items) else (1024 if len(item_ids(items)) == 24 else 512)


def fact_sources(span_count):
    return ['unknown', 'meta:적용계약법', 'meta:계약방법', 'meta:세부품명번호목록'] + [f'S{i}' for i in range(1, span_count+1)]


def fact_schema(span_count):
    return {'type':'object', 'additionalProperties':False,
        'required':list(FACT_STATES), 'properties':{
            key:{'type':'object', 'additionalProperties':False, 'required':['state','source'],
                 'properties':{'state':{'type':'string','enum':list(states)},
                               'source':{'type':'string','enum':fact_sources(span_count)}}}
            for key,states in FACT_STATES.items()}}


def mock_facts():
    return {key:{'state':'unknown','source':'unknown'} for key in FACT_STATES}


def validate_facts(facts, span_count):
    if not isinstance(facts,dict) or set(facts)!=set(FACT_STATES):
        raise ValueError('Expected exactly the B-group fact fields')
    for key, value in facts.items():
        if not isinstance(value,dict) or set(value)!={'state','source'}:
            raise ValueError('Expected fact state/source')
        if value['state'] not in FACT_STATES[key] or value['source'] not in fact_sources(span_count):
            raise ValueError('Invalid fact state or source outside current request')
...
def output_schema(span_count, items=None):
    ...
    keys=[f'v{i}' for i in item_ids(items)]
    schema = {'type':'object','additionalProperties':False,'required':keys,'properties':{
        ...
        for key in keys}}
    if uses_facts(items):
        schema['required'] = ['facts'] + keys
        schema['properties'] = {'facts':fact_schema(span_count), **schema['properties']}
    return schema
...
class VLLMRunner:
    ...
    def generate(self,messages,span_counts,items=None):
        ...
        target_items=item_ids(items)
        params=[SamplingParams(temperature=0.0,seed=SEED,max_tokens=output_limit(target_items),
...
class MockRunner:
    ...
    def generate(self,messages,span_counts,items=None):
        if len(messages)!=len(span_counts):
            raise ValueError('Each conversation requires its own source span count')
        prefix = {'facts':mock_facts()} if uses_facts(items) else {}
        return [{'text':json.dumps({**prefix, **{f'v{i}':{'violation':0,'evidence':0} for i in item_ids(items)}}),'finish_reason':'mock'} for _ in messages]


def fit_v4_messages(rec,assets,runner,max_chars=16000,examples=1,items=None,budget=PROMPT_BUDGET):
...
def fit_messages(rec,assets,runner,max_chars=16000,examples=1,items=None,budget=PROMPT_BUDGET):
    messages,spans,ids,tokens = fit_v4_messages(rec,assets,runner,max_chars,examples,items,budget)
    if uses_facts(items):
        # Append AFTER v4 fitting: no source/example changes due to new instructions.
        messages = [dict(message) for message in messages]
        messages[0]['content'] += FACT_INSTRUCTION
        tokens = runner.count(messages)
        if tokens > budget + 1536 or tokens + output_limit(items) > 16384:
            raise RuntimeError('Fact instruction exceeds reserved budget; source must not be truncated')
    return messages,spans,ids,tokens
...
def parse_response(result,span_count,mock=False,items=None):
    ...
    target_items=item_ids(items)
    if uses_facts(target_items):
        validate_facts(obj.pop('facts',None),span_count)
...
def main():
    ...
    audit={'mode':'mock' if args.mock else 'fixed_llm','input_count':len(records),
           ...
           'phase':'loading_model','output_schema':'v5_B_facts_then_named_v1_v24',
           'settings':{'prompt_budget':PROMPT_BUDGET,'max_chars':args.max_chars,
                       'examples':args.examples,'batch_size':args.batch_size,
                       'single_pass':args.single_pass,
                       'b_fact_instruction_reserve':1536,'b_output_limit':768,
                       'v4_context_preserved':True,
            ...
            for group_name,group_items,budget in groups:
                ...
                for index,(rec,prep,result) in enumerate(zip(batch,prepared,results)):
                    ...
                    try:
                        ...
                        fact_correction = 'facts를 먼저 출력하고 각 필드는 지정된 state/source만 사용. 이어서 ' if uses_facts(group_items) else ''
                        correction={'role':'user','content':fact_correction+f'{requested} 키를 사용하고 각 항목 값은 violation과 evidence를 가진 정확한 JSON. evidence는 0~{len(spans)}. 정상·부재탐지 evidence=0.'}
                        retry=messages+[{'role':'assistant','content':result.get('text','')},correction]
                        retry_limit=output_limit(group_items)
                        if runner.count(retry)+retry_limit>16384:
